...
% path = 'C:\Users\Elena\Documents\ECOG data\Frequency test GL\Channel 8';
% cd(path)
% files = dir('*.apm');
...
%                             spike_channel(i).time(ix) = spike_channel(i).time(end) + (1:length(ix));
        ...
        aux_channel = [];
        for i=1:n_aux
%             if i==11 % this if clause was used for RClarL07 (11/6/08)
%                 continue
%             else
...
%                         %------------------------------
%                         % Tippet (9/22/2009) aux chan recorded at Fs=5k.
%                         % Downsample to Fs=1k.
%                         aux_channel(j).data = downsample(aux_channel(j).data,5);
%                         aux_channel(j).time = downsample(aux_channel(j).time,5);
%                         aux_channel(j).sampfreq = aux_channel(j).sampfreq/5;
%                         %------------------------------
...
% % for Miller, ecog accidentally recorded with sampling frequency of 5kHz.  Downsample
% % from 5kHz -> 1kHz using Matlab function 'downsample'
%for i = 1:n_ecog-1
%   spike_channel(ecog_ch(i)).data = downsample(spike_channel(ecog_ch(i)).data,5);
%end

% for BeatL02, PaolL12, PillR04 rest PSD analysis, manually extract the
% first 30 sec ONLY.
% for i=1:n_ecog
%     spike_channel(ecog_ch(i)).data = spike_channel(ecog_ch(i)).data(1:30*1000);
% end

% % % for 6/30/09, downsample GL4k channel 5 from 5kHz -> 1kHz
% spike_channel(5).data = downsample(spike_channel(5).data,5);
% spike_channel(5).time = downsample(spike_channel(5).time,5);

% % Tippett 9/22/2009: GL4k ch.5 (ecog3-6) recorded at Fs=5k.  Downsample
% % to Fs=1k match sampling frequency of other ecog/LFP channels
% spike_channel(5).data = downsample(spike_channel(5).data,5);

%% downsapmle the data to 1K
for i =1:n_ecog
    d=spike_channel(ecog_ch(i)).sampfreq/1000;
    spike_channel(ecog_ch(i)).data= resample(spike_channel(ecog_ch(i)).data,1,d);
end
...
% disp(['Processing ' num2str(n_ecog) ' ecog/LFP channels']);
% ecog_chan = [];
% time = [];
% j = 1;
% for i = ecog_ch
...
%         j = j + 1;
%     else
%         disp(sprintf('WARNING: ECOG/LFP channel %d does not have the same sampling frequency like the other channels! Channel will be skipped!',i));
%     end;
% end
...
%     fluctuations in task voltage. ParseIntraOpEvents uses MARGIN = 0.05
      % 4/7/2010: Elena found that 0.20 was insufficient for
%     pt Schaffer (3/30/2010).  Use the margin below for Schaffer ONLY.
    MARGIN=0.1;
%     REST = [3.60-MARGIN 3.60+MARGIN];
     %REST = [5.25-MARGIN 5.25+MARGIN];
       %ACTIVE = [4.46-MARGIN 4.46+MARGIN];
    REST = [6.55-MARGIN 6.55+MARGIN];
%     REST = [7.8-MARGIN 7.8+MARGIN];
    %ACTIVE = [1.69-MARGIN 1.69+MARGIN];
%     ACTIVE =  [7.3-MARGIN 7.3+MARGIN];
%     ACTIVE =  [4.46-MARGIN 4.46+MARGIN];
    ACTIVE =  [6.21-MARGIN 6.21+MARGIN];
%     ACTIVE =  [8.2-MARGIN 8.2+MARGIN];
     %REST =  [5.25-MARGIN 5.25+MARGIN];
%     ACTIVE =  [3.60-MARGIN 3.60+MARGIN];
%       ACTIVE = [7.70-MARGIN 7.70+MARGIN];
%       REST = [7.30-MARGIN 7.30+MARGIN]; 
% BASELINE = 3.8; 
...
% ACTIVE =  [6.17-MARGIN 6.17+MARGIN];
    ...
    try
        ...
        % This results from large voltage fluctuations that sometimes happe1ns when task button is pressed or unpressed
