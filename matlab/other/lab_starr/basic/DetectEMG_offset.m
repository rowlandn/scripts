function emg_offset = DetectEMG(time, emg, epoch)
...
EMG_bl = [1 2];    % seconds before epoch timestamp used for baseline EMG activity
...
for i = 1:n_epoch
	...
	start_ind = max(find(abs(emg(test_inds))> THRESH*cntl_sd )); %#ok<MXFND>
    ...
    if i==1
        disp(['If offset is correct, hit return to move to next epoch.' sprintf('\n')...
            'Otherwise, correct offset as needed or click outside box to delete, then hit return.']);
    end
	[x,y] = ginput(1);
	while ~isempty(x)
		if ishandle(c)
