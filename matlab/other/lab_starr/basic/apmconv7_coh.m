...
evt_ch = input('Enter channel # for ECOG event voltage, otherwise leave blank: ');
...
minlength=min([length(spike_channel(3).data) length(spike_channel(4).data) length(spike_channel(5).data)...
    length(spike_channel(6).data) length(spike_channel(7).data) length(spike_channel(8).data)]);
spike_channel(3).data  = spike_channel(3).data(1:minlength);
spike_channel(4).data  = spike_channel(4).data(1:minlength);
spike_channel(5).data  = spike_channel(5).data(1:minlength);
spike_channel(6).data  = spike_channel(6).data(1:minlength);
spike_channel(7).data  = spike_channel(7).data(1:minlength);
spike_channel(8).data   = spike_channel(8).data(1:minlength);

%% force ecog/lfp # samples to be the same lengths for montage-ing
%Would be best to build in some kind of error message trigger in case one
%or more channels actually was accidentally sampled at a different rate
minsample=min([length(spike_channel(3).time) length(spike_channel(4).time) length(spike_channel(5).time)...
    length(spike_channel(6).time) length(spike_channel(7).time) length(spike_channel(8).time)]);
spike_channel(3).time  = spike_channel(3).data(1:minsample);
spike_channel(4).time  = spike_channel(4).data(1:minsample);
spike_channel(5).time  = spike_channel(5).data(1:minsample);
spike_channel(6).time  = spike_channel(6).data(1:minsample);
spike_channel(7).time  = spike_channel(7).data(1:minsample);
spike_channel(8).time   = spike_channel(8).data(1:minsample);
%% process and save ecog/lfp channels
...
n_lfp = length(lfp_ch);
disp(['Processing ' num2str(n_lfp) ' LFP channels']);
%%
lfp_chan = [];
%%
time = [];
...
%         ecog(1).contact_pair(j).raw_ecog_signal = spike_channel(i).data;
%         ecog(1).contact_pair(j).chan_num = i;
        lfp_chan(j,:) = spike_channel(i).data;                    % For compatibility with the original script; works only when the sampling rates on all emg channels is the same
        lfp_names(j,:) = spike_names(i,:);                        % For compatibility with the original script
        time = spike_channel(i).time;                               % For compatibility with the original script
        j = j + 1;
    else
        disp(sprintf('WARNING: LFP channel %d does not have the same sampling frequency like the other channels! Channel will be skipped!',i));
    end;
end

ecog_lfp_raw_data = [ecog_chan; lfp_chan]';

%% parse ecog rest/active timestamps using getevts
if ~isempty(evt_ch)
    time = spike_channel(evt_ch).time;
    evt_chan = spike_channel(evt_ch).data;
    MARGIN = 0.20;  % 0.35 is a very wide range that handles large fluctuations in task voltage. ParseIntraOpEvents uses MARGIN = 0.05
%     REST = [5.25-MARGIN 5.25+MARGIN]; % this is the typical rest voltage
%     ACTIVE =  [4.46-MARGIN 4.46+MARGIN]; %typical voltage during active epochs
%     ACTIVE = [1.69-MARGIN 1.69+MARGIN]; %

%     REST =  [3.60-MARGIN 3.60+MARGIN];% voltage settings for pt Paol rest
%     ACTIVE = [5.25-MARGIN 5.25+MARGIN]; % Paol active

%     REST = [1.69-MARGIN 1.69+MARGIN]; % Beaton rest voltage
%     ACTIVE = [5.25-MARGIN 5.25+MARGIN]; %Beaton active voltage

% If switch is in left side position and button is pressed/released for task, as for pt Miller (SSEP Ch 12)     
%       REST = [7.70-MARGIN 7.70+MARGIN];
%       ACTIVE = [7.30-MARGIN 7.30+MARGIN]; 
%If switch is in center position for rest, right side for task, as Morris: 
%     REST = [7.8-MARGIN 7.8+MARGIN];        
%     ACTIVE = [8.2-MARGIN 8.2+MARGIN];

%After adjusting the aux channels, a std button press voltage is as follows:  
% MARGIN = 0.20; 
...
outputname = [vn '.mat'];
disp(['Writing ECOG/LFP  channels to:  ' outputname(1:end-4) ecog_channel_append lfp_channel_append '.mat']);
...
save( [outputname(1:end-4) ecog_channel_append lfp_channel_append '.mat'], 'ecog_lfp_raw_data');

% save txt file
fname = [outputname(1:end-4) ecog_channel_append lfp_channel_append '.txt'];
outfid = fopen(fname,'w');
if(outfid == -1)
    error(['Unable to open...' fname ]);
end
for i = 1:n_ecog
    fprintf(outfid, ['ECOG GL Ch #' num2str(ecog_ch(i)) '\t']);
end

for i = 1:n_lfp
        fprintf(outfid,['LFP GL Ch #' num2str(lfp_ch(i)) '\t']);
end

fprintf(outfid,'Rest Time \t');
fprintf(outfid,'Active Time \t\r');

% [n_row n_col] = size(ecog_lfp_raw_data);
% 
% for i = 1:n_row
%     for j = 1:n_col
%         if j == n_col
%             fprintf(outfid, [num2str(ecog_lfp_raw_data(i,j)) '\t\r']);
%         else
%             fprintf(outfid, [num2str(ecog_lfp_raw_data(i,j)) '\t']);
%         end
%     end
% end
