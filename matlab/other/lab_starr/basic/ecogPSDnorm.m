...
% SAS 9/7/2009: instead of using messy for-loops like the old method, use simple
...
[allfreq subfreq order] = NORMquantPSD(rest,active,FREQ_QPSD,freq,fn,M1_ch);
...
function [allfreq subfreq order] = NORMquantPSD(rest,active,FREQ_QPSD,freq,filename,M1_ch)
...
for i=1:3
    ...
    rest_data = rest.contact_pair(order(i)).norm_mean_PSD;
    active_data = active.contact_pair(order(i)).norm_mean_PSD;
