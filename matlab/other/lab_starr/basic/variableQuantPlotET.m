function variableQuantPlotET
%This is equal to variableQuantPlot, but for comparing PD, ET, and PD with
%tremor. 
...
ETpath = uigetdir('', 'Select directory that contains ET _ecgPSD files to be analyzed');
ETpath = [ETpath '\'];
cd(ETpath);
...
Yrest(:,2) = mean(ETrest,2);
Erest(:,2)= std(ETrest,0,2);
Yactive(:,2) = mean(ETactive,2);
Eactive(:,2) = std(ETactive,0,2);
...
%     % bands as defined above
% %     [allfreq subfreq] =
%     quantPSDshort(rest,active,FREQ_QPSD,order,freq,filename);
%     %now create matrix of percent power in each freq band for each subject
%     ETrest = [ETrest subfreq(:,2,k)]; %#ok<AGROW>
%         ETactive = [ETactive subfreq(:,4,k)]; %#ok<AGROW>
%         counterET = counterET+1;
