...
event_time = 4000; % length in msec of peri-event period being analyzed
time_segment = 512; % segment of data in msec that will be analyzed by sequental coherence estimates (size of each column of the coherogram)
...
window = time_segement/8; %epoch data will be divided into chunks of size equal to window for coherence calculation, the default is to break the segment being analyzed into 8 equal chunks
noverlap = window/2; % how much overlap there is between windows
% nfft and window must be equal and noverlap is a fraction of those (50%,
% 75%, etc.
num_segment = event_time/time_segment; % number of segments within the peri-event period that will undergo independent coherence analysis
...
% This program determines the coherence of two signals around the time of movement onset. 
% Data being evaluated include 2 sec before and 2 sec after movement onset. 
% Coherence analyzed in equal length segments within the 4 second perievent
% period.  


%Create empty 3D matrix where rows = signal after window averaging (with coherence for 
% each time_segment listed sequentially down the rows), 
%columns = number of comparisons (each contact vs LFP), and 
% 3rd dimesion = number of epochs
all_epoch_meancoh_r = zeros((nfft/2+1)*num_segment,num_contact_pair-1,num_epoch);
all_epoch_meancoh_a = zeros((nfft/2+1)*num_segment,num_contact_pair-1,num_epoch);
...
%         
          inp1 = data called from sho's code, contact 1)
          inp2 = data called from sho's code, contact lfp
...
%             tmp2_r = inp2_r((k-1)*LEN_WINDOW+1 : k*LEN_WINDOW);
              ...
              tmp1 = inp1((k-1)*time_segment +1 : k*time_segment);
              tmp2 = inp2((k-1)*time_segment +1 : k*time_segment);
...
%             all_epoch_meancoh_r(:,j,i) = Cxy;
            [Cxy, F] = mscohere(tmp1,tmp2,window,noverlap,nfft,Fs);
            all_epoch_meancoh_r((k-1)*time_segment +1 : k*time_segment,j,i) = Cxy;
%         end
...
% for i = 1:15 %may make more flexible later
%     h(i) = subplot(3,5,i); % subplot((num_contact_pair-1),?,i)
% end
% 
% for i = 1:5
%     get (h(i));
...
for i = 1:num_contact_pair-1
    h = subplot(4,5,(i+10));
    plot(F, trans_coh_r(:,i), 'DisplayName', 'Rest', 'XDataSource', 'F', 'YDataSource', 'Transformed Coherence during Rest','color','b','LineWidth',1.5);
    hold on;
    plot(F, trans_coh_a(:,i), 'DisplayName', 'Active', 'XDataSource', 'F', 'YDataSource', 'Transformed Coherence during Movement','color','r','LineWidth',1.5);
    axis([0 150 0 1]) % wide view axis
%     axis([0 30 0 0.5]) %axis of alpha and beta freq
    set (gca,'xtick',[0:20:150])
%     daspect([20 1 1])
...
for i = 1:num_contact_pair-1 
    h = subplot(4,5,(i+15));
    plot(F, trans_coh_r(:,i), 'DisplayName', 'Rest', 'XDataSource', 'F', 'YDataSource', 'Mean Coherence during Rest','color','b','LineWidth',1.5);
    hold on;
    plot(F, trans_coh_a(:,i), 'DisplayName', 'Active', 'XDataSource', 'F', 'YDataSource', 'Mean Coherence during Movement','color','r','LineWidth',1.5);
%     axis([0 150 0 1]) % wide view axis
