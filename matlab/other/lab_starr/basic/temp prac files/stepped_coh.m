...
%     output_a = zeros(NFFT/2+1, num_loops_a(i));
    num_steps_r(i) = floor((epoch_length_r(i) - LEN_WINDOW)/Step) - 2; %subtract 1 because no step at beginning but still exceeds dataset
    num_steps_a(i) = floor((epoch_length_a(i) - LEN_WINDOW)/Step) - 2;
    
    output_r = zeros(NFFT/2+1, num_steps_r(i)); 
    output_a = zeros(NFFT/2+1, num_steps_a(i));
...
%     First comparing all remontaged contacts to the LFP
    ...
    for j = 1:num_contact_pair-1 % subtract one b/c 6 contacts means 5 contact pairs
        ...
        for k = 1:num_steps_r(i)
            
            if k == 1
                tmp1_r = inp1_r(1:LEN_WINDOW);
                tmp2_r = inp2_r(1:LEN_WINDOW);
            else
                tmp1_r = inp1_r(Step*(k-1)+1:Step*(k-1)+1+LEN_WINDOW);
                tmp2_r = inp2_r(Step*(k-1)+1:Step*(k-1)+1+LEN_WINDOW);
            end
...
%         hold on;
        ...
        for l = 1:num_steps_a(i)
             if k == 1
                tmp1_a = inp1_a(1:LEN_WINDOW);
                tmp2_a = inp2_a(1:LEN_WINDOW);
            else
                tmp1_a = inp1_a(Step*(l-1)+1:Step*(l-1)+1+LEN_WINDOW);
                tmp2_a = inp2_a(Step*(l-1)+1:Step*(l-1)+1+LEN_WINDOW);
             end
...
%     axis([0 30 0 0.5]) %axis of alpha and beta freq
    axis([0 50 0 0.5]) % slightly higher than beta axis
%     set (gca,'xtick',[0:20:150])
