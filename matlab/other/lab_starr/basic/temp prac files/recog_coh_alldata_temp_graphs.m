...
% apmconv7_coh
[fn pn] = uigetfile('*_ecog_lfp.mat','Select .mat containing ecog_lfp data');
cd(pn);
load([pn fn]);
...
%             epoch_ends_r(j) = epoch_length_r(j);
            ...
            epoch_starts_r(j)= 1;
            epoch_ends_r(j) = int32(EPOCH_LEN*Fs);
        else
...
trans_coh.rest = atanh(sqrt(grand_meancoh_r));
trans_coh.active = atanh(sqrt(grand_meancoh_a));
trans_coh.freq = F;
...
trans_coh.rest_M1 = M1_contact
trans_coh.active_M1= M1_contact
