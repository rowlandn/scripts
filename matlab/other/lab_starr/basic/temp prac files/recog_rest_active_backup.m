...
%Create empty matrix to hold the mean_coh values for each epoch. We will
%use this matrix later for averaging all the epochs together. 
grand_meancoh_r = zeros(NFFT/2+1,num_epoch);
grand_meancoh_a = zeros(NFFT/2+1,num_epoch);
...
%     Length_r(i) = epoch_length_r(i); % deleted due to redundancy
...
% average the epochs
all_epoch_avg_r = mean(grand_meancoh_r,2);
all_epoch_avg_a = mean(grand_meancoh_a,2);

% average across epochs of ch 5 vs lfp, rest vs active
plot(F, all_epoch_avg_r, 'DisplayName', 'Mean Coherence vs F,', 'XDataSource', 'F', 'YDataSource', 'Mean Coherence during Rest','color','b');
hold on;
plot(F, all_epoch_avg_a, 'DisplayName', 'Mean Coherence vs F,', 'XDataSource', 'F', 'YDataSource', 'Mean Coherence during Movement','color','r');
axis([0 150 0 1])
set (gca,'xtick',[0:20:150])

%
% % Now comparing adjacent contacts
% for i = 1:num_epoch
%     
%     hf = figure;
%     
