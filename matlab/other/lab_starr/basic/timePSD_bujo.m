...
%extract ecog bipolar recordings from ecog_lfp_raw_data
ecog34=ecog.contact_pair(1).raw_ecog_signal-ecog.contact_pair(2).raw_ecog_signal;
ecog45=ecog.contact_pair(2).raw_ecog_signal-ecog.contact_pair(3).raw_ecog_signal;
ecog56=ecog.contact_pair(3).raw_ecog_signal;
%concatenate such that each column contains data from one channel
ecoglfpall=[ecog34' ecog45' ecog56'];

% if LFP data exists, include in ecoglfpall
k_LFP = menu('analyze LFP?','yes','no');
if k_LFP==1
%     lfp = ecog.contact_pair(6).raw_ecog_signal;
    lfp = ecog.contact_pair(4).raw_ecog_signal;
...
if typedata==1
    save([outputname '_timePSD_emg.mat'],'A2plot', 'C_trans','faxis','taxis','move_onset','emg_i','emg_ch_names');
...
elseif typedata==2
    save([outputname '_timePSD_accel.mat'],'A2plot','C_trans','faxis','taxis','move_onset');
%     save([outputname '_timePSD_accel.mat'],'Smag_mean','faxis','taxis','move_onset');
    ...
    save([outputname '_timePSD_button.mat'],'A2plot','C_trans','faxis','taxis','move_onset');
