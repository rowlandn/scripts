function [spk,f_sum,cl] = SpikeEMG_multi(nexname, spkname_pattern)
% [spk,emg] = SpikeEMG_multi(fname, spkname_pattern)
% This program analyses Spike autocorrelations & versus EMG/Accel data
% Created by RST, 2002-04-05
% Modified by RST, 2003-10-15
% Modified by RST, 2004-01-27
% Modified by RST, 2005-07-12 to handle multi-spike NEX files
%
% Optional argument 'spkname_pattern' can be used to process only matching
% spike names
...
% Find out how many spike trains there are and in what order
spkvar_inds = find(Nexvar.Type == SPIKE_TYPE);
n_spkvars = length(spkvar_inds);

if exist('spkname_pattern')
	for i=1:n_spkvars		% Find spikes that match desired name pattern
		varname = deblank(Nexvar.Name(spkvar_inds(i),:));
		if isempty( regexpi(varname,'Snip\w*[abcd]') )
			spkvar_inds(i) = NaN;	% If names don't match, mark as bad
		end
	end
	spkvar_inds( isnan(spkvar_inds) ) = [];
	n_spkvars = length(spkvar_inds);
end
	
for i=1:n_spkvars		% Now run for matching spike
	spkname = deblank( strrep(nexname,'.nex',['_' Nexvar.Name(spkvar_inds(i),:)] ) );

	spk.t = Nexdata.TS{ spkvar_inds(i) } ;
	spk.n = Nexdata.TS_count( spkvar_inds(i) );
	...
	[sz x] = size(tmp);
	if(sz ==0)
		warning(['I can''t find the EMG file:  ' fname '. Assuming that no EMG/Accel is available.']);
		EMG_FLAG = false;
		n_emg = 1;
	else
		EMG_FLAG = true;
	...
	spk.acorr_subsamp = decim(temp,SS_INTERV);	% sub-sample autocorr down to 200 Hz
	spk.lag_subsamp = decim(spk.lag,SS_INTERV);	% sub-sample lags down to 200 Hz
	...
	if EMG_FLAG
		...
		for i = 1:n_emg
			emg(i,:) = decim(emg_chan(i,:),SS_INTERV);
		end
...
end		% End of for loop for each spike in one NEX file
