...
% Revised by SS 2008-01-17: Alignment selection performed once, output
% filenames revised to indicate alignment selection
%
%	Input:
...
MIN_SPK_N = 500;
EVENTS = {'home' 'light' 'accel' 'emg' 'move' };
...
outfid = write_text_header(N_Pks, align_select);	% Subfunction below
...
for i=1:length(FileLst)
	...
	L_inds = find(events.target == 'L');
	R_inds = find(events.target == 'R');
	eval(['tmp_evt = events.' align_select ';']);
...
fclose(outfid);
save(['PeriORTask_' align_select], 'spk');
