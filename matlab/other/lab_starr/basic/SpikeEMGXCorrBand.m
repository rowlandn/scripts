...
FS = 1000;
SRCH_LO = 0;	% low freq for search for significant XCorrs
SRCH_HI = 1;	% top freq for search for significant XCorrs
...
% Open output file and print header line
	...
	fprintf(outfile,'AC_more?\tXC_SIG\t');		% Header for missed significant ACorr peaks & XCorr 95% CI
	for i = 1:N_XCorr
...
while( m < length(FileLst))
	...
	[spk{m}, emg{m}, emg_cl{m}] = SpikeEMGXCorr(FileLst(m).name);
