function Burst = legendy_new4(ISI)
...
%
% Written 9/21-23/2001, 7/24/2003, 11/27/2003, 7/31/2005 by Thomas Wichmann.
...
while n < length(ISI) - min_length_of_burst
    ...
    % ******* 3. selection step - test whether adding up to 10 more ISIs will enhance surprise value **********************
            ...
            S = surprise_new3(mean_FR,ISI(n:n+m),sampling_rate);                                        
            ...
            if n+m < length(ISI)
                ...
                if (max_S > surprise_new3(mean_FR,ISI(n:n+m),sampling_rate)) % check whether the maximal surprise value in the post-burst period is higher than the maximal burst surprise
                    m = m+ind_max_S;
                end
            else
                m = length(ISI)-n;
        ...
        % ******** 4. selection step - test whether adjusting the front end of the burst enhances the surprise value ******************
            if n > 1,
                o = 1; 
                inc_flag = 0;
                while ((m-o > min_length_of_burst) & (surprise_new3(mean_FR,ISI(n+o:n+m),sampling_rate) >= surprise_new3(mean_FR,ISI(n+o-1:n+m),sampling_rate)))
                    o = o+1;
                    inc_flag = 1;
                end
                if inc_flag,
