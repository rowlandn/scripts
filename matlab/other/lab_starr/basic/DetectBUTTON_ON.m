function move_onset = DetectBUTTON_ON(time, signal, signal2, epoch)
...
signal_bl = [-4 -3];    % seconds before epoch timestamp used for baseline signal activity
signal_test = [-2 0]; % seconds around epoch timestamp to test signal
n_epoch = length(epoch);
move_onset = zeros(1,n_epoch);
c = [];
