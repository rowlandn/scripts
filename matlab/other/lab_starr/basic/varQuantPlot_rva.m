...
Ypd(:,1) = mean(PDrest,2);
Epd(:,1) = std(PDrest,0,2);
Ypd(:,2) = mean(PDactive,2);
Epd(:,2) = std(PDactive,0,2);
...
Ydys(:,1) = mean(DYSrest,2);
Edys(:,1) = std(DYSrest,0,2);
Ydys(:,2) = mean(DYSactive,2);
Edys(:,2) = std(DYSactive,0,2);
...
%     [allfreq subfreq] = quantPSDmatrix(rest,active,FREQ_QPSD,freq,filename);
    ...
    Yet(:,1) = mean(ETrest,2);
    Eet(:,1) = std(ETrest,0,2);
    Yet(:,2) = mean(ETactive,2);
    Eet(:,2) = std(ETactive,0,2);
...
% Yother(:,1) = mean(OTHERrest,2);
% Eother(:,1) = std(OTHERrest,0,2);
% Yother(:,2) = mean(OTHERactive,2);
% Yother(:,2) = std(OTHERactive,2);
% end
...
if ET
    subplot(3,1,1);
    handlespd = barEB(100*Ypd, 100*Epd);
    set(handlespd.ca,'XTick',[1;2;3;4;5],...
        'XTickLabel',FREQBANDS,...        
        'ylim',YLIM,...
        'YMinorGrid','on');
    title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in PD (n=' num2str(counterPD) ')']);
%     title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in PD: 13-30Hz (n=' num2str(counterPD) ')']);
    ylabel('% power');
    hREST=handlespd.bars(1);
    hACTIVE=handlespd.bars(2);
%     hET=handlesr.bars(3)
    set(hREST,'FaceColor','b');
    set(hACTIVE,'FaceColor','r');
    legend(['PD rest'],...
            ['PD active']);
        
    subplot(3,1,2);
    handlesdys=barEB(100*Ydys,100*Edys);
    set(handlesdys.ca,'XTick',[1;2;3;4;5],...
        'XTickLabel',FREQBANDS,...
        'ylim',YLIM,...
        'YMinorGrid','on');
    title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in DYS (n=' num2str(counterDYS) ')']);
%     title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in DYS: 13-30Hz (n=' num2str(counterPD) ')']);
    ylabel('% power');
    hREST=handlesdys.bars(1);
    hACTIVE=handlesdys.bars(2);
    set(hREST,'FaceColor','b');
    set(hACTIVE,'FaceColor','r');
    legend(['DYS rest'],...
            ['DYS active']);
    ...
    subplot(3,1,3);
    handleset=barEB(100*Yet,100*Eet);
    set(handleset.ca,'XTick',[1;2;3;4;5],...
        'XTickLabel',FREQBANDS,...
        'ylim',YLIM,...
        'YMinorGrid','on');
    title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in ET (n=' num2str(counterET) ')']);
%     title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in ET: 13-30Hz (n=' num2str(counterPD) ')']);
    ylabel('% power');
    hREST=handleset.bars(1);
    hACTIVE=handleset.bars(2);
    set(hREST,'FaceColor','b');
    set(hACTIVE,'FaceColor','r');
    legend(['ET rest'],...
            ['ET active']); 
        ...
        subplot(2,1,1);
    handlesr = barEB(100*Ypd,100*Epd);
    ...
    title([BRAINAREAS{k} ' with ' LIMBAREAS{l} ' movement in PD (n=' num2str(counterPD) ')']);
    ylabel('% power');
    hREST=handlesr.bars(1);
    hACTIVE=handlesr.bars(2);
    set(hREST,'FaceColor','b');
    set(hACTIVE,'FaceColor','r');
    legend(['PD rest'],...
            ['PD active']);
    ...
    subplot(2,1,2);
    handlesa=barEB(100*Ydys,100*Edys);
