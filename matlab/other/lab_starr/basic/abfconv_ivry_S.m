...
%
%   Info from Channel 2 (Channel 10 on the Axon) are saved in "fname.mat"
%   for subsequent merging with spike times.  This channel is considered
%   to contain binary 5V task data for the Ivry stop signal task.
%
...
%
% RLM: created by Rob Turner, last edited 2007.14.03 by John Schlerf
% (schlerf@gmail.com)
% Revised to parse the Ivry Stop Signal task
%
% Note: This version is specially tuned for subject SCHW.
...
% JS - Don't get this, but OK.
fn_dir = strrep(which('abfconv_ivry_S'),'abfconv_ivry_S.m','');   % Store function location for later use
...
if size(who('*Time')) > 0
   ...
   SamplingRate = round(length(time)/time(end)); % Sampling rate
   RATE1 = SamplingRate/4000;  % Output EMG is first sub-sampled at 1/RATE1
   RATE2 = 4000/1000;  % Output EMG is sub-sampled later at 1/RATE2
   RATE_PLT = SamplingRate/1000;
else
   ButtonName = questdlg('Time was not imported! Assuming 50 microsec. sampling interval.');
   SamplingRate = 20000;         % Sampling rate
...
%%%%%%%%%%%%%%%%%%%%
% JS - I don't have these; so I commented this out.
...
%     wavwrite((1/rng)*flt_unit,SamplingRate,16,[outputname '.wav']);

   % RLM: ##################################################################
   ddtdata = (32767/(max(abs(flt_unit))))*flt_unit; % +/-2047 value is from DDT format 12bit res.
   writeddt(ddtdata,SamplingRate,outputname,16,'Creating ddt file.');
   % RLM: ##################################################################
...
% steal TaskChannelData as a separate variable, avoid downsampling
eval(['TaskChannelData = ' tmp(evt_ch).name ';']);
TaskChannelTimeStamps = time; % Save a non-downsampled copy
...
fprintf('Processing  %d  EMG/Task/Accel channels', n_aux);
if ~(isempty(accel_ch) & isempty(emg_ch))
    for i = 1:n_aux
        ch = eval(tmp(i).name);
...
%%%%%%%%%%%%%%%%%%%
% JS - This is the routine that's special for subject SCHW
events = IvryTaskParser_S(TaskChannelTimeStamps,TaskChannelData,SamplingRate);
...
if ~isempty(events) & ~isempty(accel_ch) & accel_ch>0 & accel_ch<=n_aux
   ...
   events = DetectAccels(time,aux_chan(align_select,:),events);
end
...
% line 194 allows user to input desired alignment channel, the rest of
