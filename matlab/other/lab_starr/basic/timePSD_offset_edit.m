...
if ~isempty(emg_files) || ~isempty(accel_files)
    ...
    if usedata==1
        ...
        % both emg and accel onset times exist
        if ~isempty(emg_files) && ~isempty(accel_files)
...
%         epoch_ts = ecog.active_time;
        ...
        % find rest epoch timestamps and remove starting rest epoch from
        % analysis
        epoch_ts = ecog.rest_time; 
        if epoch_ts(1) == 0;
            epoch_ts = epoch_ts(2:end);
        end
        ...
        if typedata~=3
            ...
            %created move_offset=DetectEMG_off(time,move,epoch_ts) as
            %function file 12/14/09. ALC
            move_offset = DetectEMG_off(time,move,epoch_ts); 
            move_offset = move_offset(~isnan(move_offset));
        else
            move_offset = epoch_ts;
        end
...
%             emg_ch_names = {'CEMG1___Deltoid','CEMG2___Biceps','CEMG3___ECR'};
            emg_ch_names = {'CEMG1','CEMG2','CEMG3','CEMG4'};
%             emg_ch_names = {'CDeltoid','CBiceps','CECR'};
            emg_i = menu('Select emg channel',emg_ch_names);
...
% remove movement offset that doesn't have enough time for movement
% parsing
...
for i = 1:n_data_ch
    data = ecoglfpall(:,i);
    for j=1:n_epochs
        % take snippet of data around emg_offset from selected ecog/LFP channel
        % add offset to increase # windows for fft
        first = int32(Fs*(move_offset(j)-(PRE))-WINDOW/2); % WINDOW/2 offset will make spectrogram start at move_offset-PRE at appropriately centered PSD
...
if PRE<BL(1)
    error(['The baseline period for PSD plot currently begins '...
        '%d seconds before offset of movement. This value cannot be more than %d seconds '...
...
% SAS 6/9/2009: 'A2plot' included in output file, 'Smag_mean' (the
...
% save variables and figure
    ...
    %ALC 12/15/09: adjusted filenames for movmement offset (beta rebound)
    %condition
if n_data_ch==6  
    if typedata==1
        save([outputname '_timePSD_emg_offsetEDIT.mat'],'A2plot', 'C_trans','faxis','taxis','move_offset','emg_i','emg_ch_names');
    %     save([outputname '_timePSD_emg.mat'],'Smag_mean','faxis','taxis','move_onset','emg_i','emg_ch_names');
        saveas(hf1,[outputname '_timePSD_emg_offsetEDIT'],'fig');
        saveas(hf2,[outputname '_timeCOH_emg_offsetEDIT'],'fig');
    elseif typedata==2
        save([outputname '_timePSD_accel_offsetEDIT.mat'],'A2plot','C_trans','faxis','taxis','move_offset');
    %     save([outputname '_timePSD_accel.mat'],'Smag_mean','faxis','taxis','move_onset');
        saveas(hf1,[outputname '_timePSD_accel_offsetEDIT'],'fig');
        saveas(hf2,[outputname '_timeCOH_accel_offsetEDIT'],'fig');
    else
        save([outputname '_timePSD_button_offsetEDIT.mat'],'A2plot','C_trans','faxis','taxis','move_offset');
        saveas(hf1,[outputname '_timePSD_button_offsetEDIT'],'fig');
        saveas(hf2,[outputname '_timeCOH_button_offsetEDIT'],'fig');
    end
else
        if typedata==1
        save([outputname '_timePSD_emg_offsetEDIT.mat'],'A2plot', 'faxis','taxis','move_offset','emg_i','emg_ch_names');
    %     save([outputname '_timePSD_emg.mat'],'Smag_mean','faxis','taxis','move_onset','emg_i','emg_ch_names');
        saveas(hf1,[outputname '_timePSD_emg_offsetEDIT'],'fig');
%         saveas(hf2,[outputname '_timeCOH_emg'],'fig');
    elseif typedata==2
        save([outputname '_timePSD_accel_offsetEDIT.mat'],'A2plot','faxis','taxis','move_offset');
    %     save([outputname '_timePSD_accel.mat'],'Smag_mean','faxis','taxis','move_onset');
        saveas(hf1,[outputname '_timePSD_accel_offsetEDIT'],'fig');
%         saveas(hf2,[outputname '_timeCOH_accel'],'fig');
    else
        save([outputname '_timePSD_button_offsetEDIT.mat'],'A2plot','faxis','taxis','move_offset');
        saveas(hf1,[outputname '_timePSD_button_offsetEDIT'],'fig');
