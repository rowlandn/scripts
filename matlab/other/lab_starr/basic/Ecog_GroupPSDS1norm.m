function Ecog_GroupPSDS1norm
% Definition: same as Ecog_GroupPSD, except bars represent the log power
% in each frequency band normalized by power from a distant brain region 
% (contact pair behind S1), rather than the percent power relative to the
...
% CALLS function 'quantPSDshort_logS1norm' - this is nearly identical to the
...
% The purpose of calling 'quantPSDshort_logS1norm'is to recalcuate the allfreq and
...
% 'quantPSDshort_logS1norm' also does not write data to an excel file.
...
for i=1:numPD
    ...
    elseif order(1)<=3
        continue; % skip pts that do not have contact pair behind S1
    end
    ...
    [allfreq subfreq] = quantPSDshort_logS1norm(rest,active,FREQ_QPSD,order,freq,filename);
    ...
    PDrest = [PDrest subfreq(:,2,k)];%#ok<AGROW> %2nd column contains normalized power at rest
    PDactive = [PDactive subfreq(:,4,k)]; %#ok<AGROW> %4rd column contains normalized power with movement
