...
k_snr_th        = 0.04;  % A coorection factor for the wavelet spike removal threshold that weigths in the signal-to-noise ratio
...
%clear java
%javaclasspath
javaaddpath([pwd '/ledatastream.jar']);
...
%query=['SELECT * FROM Procedures INNER JOIN Patients ON Procedures.PatientID=Patients.PatientID WHERE FirstName=''' firstName ''''];
%query=['SELECT * FROM Passes INNER JOIN Procedures ON Passes.ProcedureID=Procedures.ProcedureID INNER JOIN Patients ON Procedures.PatientID=Patients.PatientID WHERE FirstName=''' firstName ''''];
%query=['SELECT * FROM Tracks INNER JOIN Passes ON Tracks.PassID=Passes.PassID INNER JOIN Procedures ON Passes.ProcedureID=Procedures.ProcedureID INNER JOIN Patients ON Procedures.PatientID=Patients.PatientID WHERE FirstName=''' firstName ''''];
%query=['SELECT * FROM Events INNER JOIN Tracks ON Events.TrackID=Tracks.TrackID INNER JOIN Passes ON Tracks.PassID=Passes.PassID INNER JOIN Procedures ON Passes.ProcedureID=Procedures.ProcedureID INNER JOIN Patients ON Procedures.PatientID=Patients.PatientID WHERE Patients.FirstName=''' firstName ''' AND Patients.LastName=''' lastName ''' AND Procedures.ProcedureName=''' procedure ''' AND Passes.PassName=''' pass  ''' AND Tracks.TrackName=''' track ''''];
%query=['SELECT *, WavID = CONVERT(nvarchar(36),WaveformID) FROM Events INNER JOIN Tracks ON Events.TrackID=Tracks.TrackID INNER JOIN Passes ON Tracks.PassID=Passes.PassID INNER JOIN Procedures ON Passes.ProcedureID=Procedures.ProcedureID INNER JOIN Patients ON Procedures.PatientID=Patients.PatientID WHERE Patients.FirstName=''' firstName ''' AND Patients.LastName=''' lastName ''' AND Procedures.ProcedureName=''' procedure ''' AND Passes.PassName=''' pass  ''' AND Tracks.TrackName=''' track ''''];
%query=['SELECT Events.Caption, WavID = CONVERT(nvarchar(36),Waveforms.WaveformID), Waveforms.WaveformData FROM Waveforms INNER JOIN Events ON Waveforms.WaveformID=Events.WaveformID INNER JOIN Tracks ON Events.TrackID=Tracks.TrackID INNER JOIN Passes ON Tracks.PassID=Passes.PassID INNER JOIN Procedures ON Passes.ProcedureID=Procedures.ProcedureID INNER JOIN Patients ON Procedures.PatientID=Patients.PatientID WHERE Patients.FirstName=''' firstName ''' AND Patients.LastName=''' lastName ''' AND Procedures.ProcedureName=''' procedure ''' AND Passes.PassName=''' pass  ''' AND Tracks.TrackName=''' track ''''];
...
while (rs.next)
    ...
    microDistanceToTarget = -double(rs.getDouble('MicroDistanceToTarget'));
    macroDistanceToTarget = -double(rs.getDouble('MacroDistanceToTarget'));
    ...
    if (~isempty(waveformID) && ~strcmp(waveformID,'00000000-0000-0000-0000-000000000000'))
        ...
        if (abs(t.channels(channel).win_high)>5*maxw)
        ...
        try 
            events(eventNum).timestamps = (t.channels(channel).timestamps - t.channels(channel).start_continuous) * t.channels(channel).time_calibration * 1e-3;    % In msecs
        end;
