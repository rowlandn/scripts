function Ecog_GroupPSDrestgnorm()
...
%     [allfreq subfreq] = quantPSDmatrix(rest,active,FREQ_QPSD,freq,filename);
    ...
    PDrest = [PDrest subfreq(:,3,k)]; %#ok<AGROW> %3rd column contains gamma normalized power at rest
...
%     [allfreq subfreq] = quantPSDmatrix(rest,active,FREQ_QPSD,freq,filename);
    ...
    %now create matrix of percent power in each freq band for each subject
    DYSrest = [DYSrest subfreq(:,3,k)]; %#ok<AGROW>
...
%     [allfreq subfreq] = quantPSDmatrix(rest,active,FREQ_QPSD,freq,filename);
    %now create matrix of percent power in each freq band for each subject
    ETrest = [ETrest subfreq(:,3,k)]; %#ok<AGROW>
...
%     [allfreq subfreq] = quantPSDmatrix(rest,active,FREQ_QPSD,freq,filename);
    %now create matrix of percent power in each freq band for each subject
    OTHERrest = [OTHERrest subfreq(:,3,k)]; %#ok<AGROW>
...
outputfn=['Ecog_GroupPSDrestgnorm_' BRAINAREAS{k}];
