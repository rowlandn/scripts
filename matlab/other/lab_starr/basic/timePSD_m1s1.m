...
% SAS 5/17/2010: adapted from timePSD code to look at M1-S1 coherence.
% TimePSD_m1s1 does NOT create ecog spectrograms.
%% initialize
...
if (isempty(emg_files) && isempty(accel_files)) || usedata == 2
    ...
    elseif typesystem == 2; % AlphaOmega recording
        ...
        if typedata==1 % process emg data
            emg_ch_names = {'CEMG1___Deltoid','CEMG2___Biceps','CEMG3___ECR','CEMG4___Gastroc'};
%             emg_ch_names = {'CDeltoid','CBiceps','CECR'}; % earlier Alpha Omega cases
%             have different EMG channel names
...
% remove movement onset that doesn't have enough time for pre-movement
% parsing
...
% calculate time-varying M1-S1 coherence
m1data = ecoglfpall(:,M1_ch);
s1data = ecoglfpall(:,M1_ch-2);

for j = 1:n_epochs
    first = int32(Fs*(move_onset(j)-(PRE))-WINDOW/2);% WINDOW/2 offset will make spectrogram start at moveonset-PRE at appropriately centered PSD
    last = int32(Fs*(move_onset(j)+(POST+ADD))-WINDOW/2);
    m1snip = m1data(first:last);
    s1snip = s1data(first:last);
    counter=1;
    coh_store=[];
    for k=1:nframes
        x = m1snip(counter:counter+WINDOW-1);
        y = s1snip(counter:counter+WINDOW-1);
...
% plot coherence
hf2 = figure;
val3 = min(min(C_trans(1:100,:)));
val4 = max(max(C_trans(1:100,:)));
clims2 = [val3 val4];
f=linspace(0,Fs/2,size(C_trans,1));

hold(gca,'on');
tmp2 = C_trans(1:100,:);%chopping C_trans will allow the whole colobar to be represented
f_new=f(1:100);
...
if typedata==1
    title([outputname sprintf('\n')...
        ...
        'M1-S1 aligned to EMG Ch.' emg_ch_names{emg_i}]);
elseif typedata==2
    title([outputname sprintf('\n')...
        '# epochs=' num2str(n_epochs) sprintf('\n')...
        'M1-S1 aligned to accel']);
else
    title([outputname sprintf('\n')...
        '# epochs=' num2str(n_epochs) sprintf('\n')...
        'M1-S1 aligned to task button']);
end
...
% if n_data_ch==6  
%     if typedata==1
%         save([outputname '_timePSD_emg.mat'],'A2plot', 'C_trans','faxis','taxis','move_onset','emg_i','emg_ch_names','M1_ch');
...
%     elseif typedata==2
%         save([outputname '_timePSD_accel.mat'],'A2plot','C_trans','faxis','taxis','move_onset','M1_ch');
...
%     else
%         save([outputname '_timePSD_button.mat'],'A2plot','C_trans','faxis','taxis','move_onset','M1_ch');
%         saveas(hf1,[outputname '_timePSD_button'],'fig');
%         saveas(hf2,[outputname '_timeCOH_button'],'fig');
%     end
% else
%         if typedata==1
%         save([outputname '_timePSD_emg.mat'],'A2plot', 'faxis','taxis','move_onset','emg_i','emg_ch_names','M1_ch');
%     %     save([outputname '_timePSD_emg.mat'],'Smag_mean','faxis','taxis','move_onset','emg_i','emg_ch_names');
%         saveas(hf1,[outputname '_timePSD_emg'],'fig');
% %         saveas(hf2,[outputname '_timeCOH_emg'],'fig');
%     elseif typedata==2
%         save([outputname '_timePSD_accel.mat'],'A2plot','faxis','taxis','move_onset','M1_ch');
%     %     save([outputname '_timePSD_accel.mat'],'Smag_mean','faxis','taxis','move_onset');
%         saveas(hf1,[outputname '_timePSD_accel'],'fig');
% %         saveas(hf2,[outputname '_timeCOH_accel'],'fig');
%     else
%         save([outputname '_timePSD_button.mat'],'A2plot','faxis','taxis','move_onset','M1_ch');
%         saveas(hf1,[outputname '_timePSD_button'],'fig');
% %         saveas(hf2,[outputname '_timeCOH_button'],'fig');
%     end
% end
