...
function htkmatconv_epilepsy(filename,M1_ch1,M1_ch2)
...
for abc =1 : length(files)
    if ~isempty(strfind(files(abc).name,'htk'))
        ...
        elseif ~isempty(strfind(file_name,'signal.htk'))
            ...
            ecog(1).contact_pair(abc).raw_ecog_signal=resample(d,2^10,5^5)*gain;
        end
        abc
...
% close all
%% common reference
length_CAR = length(ecog.contact_pair);
