function parse_NS_HDF5(filename)
...
% find_filename_02 = strfind(filename,'.hdf5');
...
info = ns_open_file(filename);

no_trials = size(info.Trials,2);
...
for i = 1:no_trials
    log{i+1,1} = info.Trials{i}.SequenceName;
    log{i+1,2} = info.Trials{i}.SetName;
    log{i+1,3} = info.Trials{i}.AcquisitionData{1}.SamplingRate;
