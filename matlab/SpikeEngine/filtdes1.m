function varargout = filtdes1(varargin)
...
%   $Revision: 1.17 $ $Date: 2002/06/13 19:37:05 $
...
assignin('base','ud',ud)
    ...
    case 'mousezoom'
        ...
        if state == 1   % button is currently down
            set(fig,'windowbuttondownfcn','filtdes1(''zoom'',''mousedown'')')
            ud.pointer = 1;  
            set(fig,'userdata',ud)
            setptr(fig,'arrow')
        else   % button is currently up - turn off zoom mode
            set(fig,'windowbuttondownfcn','filtdes1(''down'')')
            if ~isempty(ud.defaultModeObject)
                ud.defaultModeObject.value = 1;
            else
                setptr(fig,'arrow')
...
%         end
        ...
        % Convert object array of handlesopen_axes = get(gca,'position')
        open_axes = obj2vect2(ud.Objects.fdax,'h');
