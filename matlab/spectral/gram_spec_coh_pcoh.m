function gram_spec_coh_pcoh(filename,TS1,TS1_descrip,TS2,TS2_descrip,TS3,TS3_descrip);
...
figure;set(gcf,'Position',[20 50 1000 500])
   subplot(4,15,1:5)
   imagesc(timebase,freq,10*log10(sp3))
   axis xy
   set(gca,'ylim',rang,'xtick',[])
   ylabel('freq (Hz)')
   title([filename,' ',TS3_descrip])
   subplot(4,15,6:7)
   ...
   subplot(4,15,53:58)
   imagesc(timebase,freq,abs(pcoh).^2)
   axis xy
   ylabel('freq (Hz)')
   xlabel('time (s)')
